...
class COCO(data.Dataset):
	def get_ann(self, index):
		...
		for ann in anns:
			...
			cls = int(self.cat_ids[ann['category_id']])
			anns_list.append([bbox[0],bbox[1],bbox[2],bbox[3],cls])
		return anns_list
	...
	def prepare_input(self, img_path, anns):
		...
		if self.opt.keep_res:
			input_h = ((height - 1) | self.opt.pad) + 1
	...
	def __init__(self, opt, split="train"):
		...
		self.voc_color = [(v // 32 * 64 + 64, (v // 8) % 4 * 64, v % 8 * 32) \
						  for v in range(1, opt.num_classes + 1)]
