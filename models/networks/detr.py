...
from utils.detr_utils import nested_tensor_from_tensor_list, NestedTensor
...
def build_backbone(opt):
	position_embedding = build_position_encoding(opt)
	train_backbone = opt.lr_backbone > 0
	return_interm_layers = opt.masks
	backbone = Backbone(opt.backbone, train_backbone, return_interm_layers, opt.dilation)
...
class DETR(nn.Module):
	...
	def __init__(self, backbone, transformer, num_classes, num_queries, aux_loss=False):
		...
		self.class_embed1 = nn.Linear(hidden_dim, num_classes + 1)
	...
	def forward(self, samples: NestedTensor):
		...
		assert mask is not None
		hs = self.transformer(self.input_proj(src), mask, self.query_embed1.weight, pos[-1])[0]

		outputs_class = self.class_embed1(hs)
...
def detr_model(opt):
	...
	transformer = build_transformer(opt)
	model = DETR(
		backbone,
		transformer,
