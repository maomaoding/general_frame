...
class Lane_classifier(nn.Module):
	def __init__(self, num_labels=4, export_onnx=False):
		...
		self.linear2 = nn.Linear(128, num_labels*5) #4 lane 5 labels
...
#ERFNet
class Net(nn.Module):
	def __init__(self, num_classes, num_labels, export_onnx):
		super().__init__()
		self.export_onnx = export_onnx
		self.softmax = nn.Softmax(dim=1)
		self.encoder = Encoder(num_classes)
		self.lane_classifier = Lane_classifier(num_labels, export_onnx)
	...
	def forward(self, input, only_encode=False):
		if only_encode:
			...
			lane_classes = self.lane_classifier(output)
