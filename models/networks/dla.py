...
class DLASeg(nn.Module):
    def __init__(self, base_name, heads,export_onnx,
    ...
    def forward(self, x):
        ...
        if not self.export_onnx:
            ...
            return ret
        else:#导出onnx不支持模型返回值是dict，为了方便模型转换tensorRT,将后处理的sigmoid和maxpooling也加入模型
            ret = []
            self.out_stats={}
            for idx,head in enumerate(self.heads):
                #ret[head] = self.__getattr__(head)(x)
                self.out_stats[head]=idx
                ret.append(self.__getattr__(head)(x))
            kernel=3
            pad = (kernel - 1) // 2
            ret[self.out_stats["hm"]]=nn.functional.sigmoid(ret[self.out_stats["hm"]])
            hmax = nn.functional.max_pool2d(
                ret[self.out_stats["hm"]], (kernel, kernel), stride=1, padding=pad)
            ret.append(hmax)
            self.out_stats["max"]=3
            return ret
