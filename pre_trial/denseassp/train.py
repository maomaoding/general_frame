...
class Trainer():
	def __init__(self):
		...
		self.model = get_denseaspp(pretrained_weight='./snapshot/pre_denseaspp_curbstonedata_best_model.pth')
	...
	def train(self):
		for epoch in range(0, self.max_epoch):
			...
			for i, (images, targets) in enumerate(self.train_loader):
				...
				# 	weight=torch.tensor([0.2, 1.5]).to(self.device))
				#cross entropy
	...
	def save_checkpoint(self,is_best=False):
		...
		filename = '{}_{}.pth'.format('denseaspp', 'curbstonedata')
		save_path = os.path.join(directory, filename)
		torch.save(self.model.state_dict(), save_path)
		if is_best:
			best_filename = '{}_{}_best_model.pth'.format('denseaspp', 'curbstonedata')
